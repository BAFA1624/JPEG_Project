...
test_crc_iend( const std::filesystem::directory_entry & data_src ) {
    ...
    std::ifstream iend_sample_file{ data_src.path(),
    ...
    iend_sample_file.seekg( std::ios_base::end );
    const auto data_sz{ static_cast<std::size_t>( iend_sample_file.tellg() ) };
    iend_sample_file.seekg( std::ios_base::beg );
    ...
    constexpr std::bitset<CRC::crc_bits> true_iend_crc{
        "10000010011000000100001010101110"
    };

    std::cout << "Calculated crc: " << calculated_crc << std::endl;
    std::cout << "True crc: " << true_iend_crc << std::endl;

    return calculated_crc == true_iend_crc;
}
