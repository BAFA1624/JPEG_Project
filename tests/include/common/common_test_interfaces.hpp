 ...
 *       passed from the TestEnvironment for a single test, *       passing
 * outputs & errors back to the TestEnvironment.
...
concept Test = requires( TestType non_const_test, const TestType const_test,
    ...
    { const_test.run() } -> std::same_as<bool>;
    {
        const_test.get_output()
    } -> std::same_as<std::expected<TestOutput, ErrorType>>;
};

/* --- TestLoader Concept Definition --- */

/* --- TestEnvironment Concept Definition --- */

/* --- TestList Concept Definition --- */

/* --- TestRunner Concept Definition --- */
